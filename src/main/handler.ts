...
import type { CheckResult, ExportedFile, MainToUi, UiToMain } from '../shared/messages';
...
/**
 * Everything the sandbox does in answer to the UI, with no side effects at
 * import: `index.ts` wires it to `figma.ui`, and the self-test plugin
 * (tools/self-test) drives the same handler with real messages against a real
 * document, recording what it sends.
 */
export interface Handler {
  handle(message: UiToMain): Promise<void>;
  /** Tells the UI whether anything is selected. */
  reportSelection(): void;
}

export function createHandler(send: (message: MainToUi) => void): Handler {
  /**
